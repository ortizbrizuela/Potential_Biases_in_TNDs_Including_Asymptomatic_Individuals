...
*Note*: the "cf" and "mc" arguments are switches that enable the generation of "counterfactual" dfs and a variable representing measured infection status, respectively. 
...
datagen = function(cf = F, # cf = counterfactual
                   ...
                   seed = 123, 
                   OR_HV = 1.81, # beta 2
                   OR_HI = 0.9,  # gamma 1,2 and gamma 2,2
                   OR_IS = 10, # delta 4 and delta 5
                   OR_IVS = 0.52, # delta 6
                   OR_HT = 1.3, # theta 2 
                   OR_VT = 0.92, # theta 3
                   OR_ST = 6, # theta 4
                   T_Intercept = -38.8, # theta 0
                   I2_Intercept = -5.4 # gamma 2,0
                   ){  
  ...
  mutate( 
      ...
      H = rbinom(n=n(), 
                 size=1,
                 prob = plogis(-19.4 + # alpha 0
                                 log(1.59)*C)), # alpha 1
      
      V = rbinom(n=n(), 
                 size=1, 
                 prob = plogis(-12.25 + # beta 0
                                 log(1.36)*C + # beta 1
                                 log(OR_HV)*H)), # beta 2
      
      I1 = rbinom(n=n(), 
                  size=1, 
                  prob = plogis(-6.7 + # gamma 1,0
                                  log(1.12)*C + # gamma 1,1
                                  log(OR_HI)*H)), # gamma 1,2
      
      I2 = rbinom(n=n(), 
                  size=1, 
                  prob = plogis(I2_Intercept + # gamma 2,0
                                  log(1.12)*C + # gamma 2,1
                                  log(OR_HI)*H + # gamma 2,2
                                  log(0.1)*V))* # gamma 2,3
           (1-I1),  
      
      S = rbinom(n=n(), 
                 size = 1, 
                 prob = plogis(-33.8 + # delta 0
                                 log(1.2)*C + # delta 1
                                 log(1.93)*C*I1 + # delta 2
                                 log(1.93)*C*I2 +  # delta 3
                                 log(OR_IS)*I1 + # delta 4
                                 log(OR_IS)*I2 + # delta 5
                                 log(OR_IVS)*I2*V))* # delta 6
           (I1+I2), 
      
      
      H0S1 = ifelse(H==0 & S==1, # dummy variable for T
                    1,0), 
      
      H1S1 = ifelse(H==1 & S==1,  # dummy variable for T
                    1,0),
      
      T = rbinom(n=n(), 
                 size = 1, 
                 prob = plogis(T_Intercept + # theta 0
                                 log(2.47)*C + # theta 1
                                 log(OR_HT)*H + # theta 2 
                                 log(OR_VT)*V + # theta 3
                                 log(OR_ST)*S)) * # theta 4
    ...
    if(cf == T) { # cf stands for counterfactual (this is a "switch")
      ...
      df = df %>% mutate(
        ...
        # I2 Had everyone been vaccinated ************************************
        ...
        I21 = rbinom(n=n(), 
                    ...
                    prob = plogis(I2_Intercept + # gamma 2,0
                                    log(1.12)*C + # gamma 2,1 
                                    log(OR_HI)*H + # gamma 2,2
                                    log(0.1)*1))* # # gamma 2,3; replaced V by 1
              (1-I1), 
        
        
        # I2 Had no one been vaccinated **************************************
        ...
        I20 = rbinom(n=n(), 
                    ...
                    prob = plogis(I2_Intercept + # gamma 2,0
                                    log(1.12)*C + # gamma 2,1
                                    log(OR_HI)*H + # gamma 2,2
                                    log(0.1)*0))* # # gamma 2,3; replaced V by 0
              (1-I1),  
                                    
        
        # S Had everyone been vaccinated *************************************
        ...
        S1 = rbinom(n=n(), 
                   ...
                   prob = plogis(-33.8 + # delta 0
                                   log(1.2)*C + # delta 1
                                   log(1.93)*C*I1 + # delta 2
                                   log(1.93)*C*I21 +  # delta 3; replace I2->I21
                                   log(OR_IS)*I1 + # delta 4;  
                                   log(OR_IS)*I21 + # delta 5; replace I2->I21
                                   log(OR_IVS)*I21*1))* # d6; re. I2->I21 & V->1
             (I1+I21), # replace I2 by I21

        
        # S Had no one been vaccinated ****************************************
        ...
        S0 = rbinom(n=n(), 
                   ...
                   prob = plogis(-33.8 + # delta 0
                                   log(1.2)*C + # delta 1
                                   log(1.93)*C*I1 + # delta 2
                                   log(1.93)*C*I20 + # delta 3; repl. I2->I20
                                   log(OR_IS)*I1 + # delta 4 
                                   log(OR_IS)*I20 + # delta 5; repl. I2->I20
                                   log(OR_IVS)*I20*0))* # d6; I2->I20 and V->0
             (I1+I20), # Replace I2 by I20
        
        
      
        # T Had everyone been vaccinated **************************************
        
        H0S1_1 = ifelse(H==0 & S1==1, # replace S by S1
                        1,0), 
      
        H1S1_1 = ifelse(H==1 & S1==1, # replace S by S1
                        1,0),
        ...
        T1 = rbinom(n=n(), 
                 ...
                 prob = plogis(T_Intercept + # theta 0
                                 log(2.47)*C + # theta 1
                                 log(OR_HT)*H + # theta 2 
                                 log(OR_VT)*1 + # theta 3 - replaced by 1
                                 log(OR_ST)*S1)) * # theta 4 - reblaced by S1
           (1 - H1S1_1) * (1 - H0S1_1) + H1S1_1, # replace H0S1 and H1S1

        
        # T had no one been vaccinated ****************************************  
        
        H0S1_0 = ifelse(H==0 & S0==1, # replace S by S0
                        1,0), 
      
        H1S1_0 = ifelse(H==1 & S0==1, # replace S by S0
                        1,0),
        ...
        T0 = rbinom(n=n(), 
                 ...
                 prob = plogis(T_Intercept + # theta 0
                                 log(2.47)*C + # theta 1
                                 log(OR_HT)*H + # theta 2 
                                 log(OR_VT)*0 + # theta 3 - replaced by 0
                                 log(OR_ST)*S0)) * # theta 4 - reblaced by S0
           (1 - H1S1_0) * (1 - H0S1_0) + H1S1_0
...
Note: In our simulation, since there is no effect measure modification by C, the conditional RR for either symptomatic infection or infection regardless of symptoms simplifies to the marginal RR for symptomatic infection and infection, respectively. Empirical proof of this is provided in step 5-i below.
...
# 2. Gen. a variable that stands for S:I:T had everyone been vaccinated (Y1)

counterfactual$Y1 = counterfactual$S1*counterfactual$I21*counterfactual$T1
    
# 3. Gen. a variable that stands for S:I:T had no one been vaccinated (Y0)

counterfactual$Y0 = counterfactual$S0*counterfactual$I20*counterfactual$T0
...
1) Create a data frame to store the results of all the preceding steps (i.e., steps 2-4 above) but with the altered parameter (e.g., OR HV = x).
...
Thus, the final output is a list containing 'n' data frames (with 'n' equal to the number of chosen values for each varied parameter) with the log(OR) and its SE estimates. Moreover, in each data frame, each row corresponds to the results after analyzing a simulated dataframe, each of them with a different seed, but with the same value for the varied parameter for that dataframe.
...
get_cfs <- function(parametros = NULL, # Vector w/parameters to be varied
  ...
  df.sec$Y1 = df.sec$S1*df.sec$I21*df.sec$T1 # Y^a=1
  df.sec$Y0 = df.sec$S0*df.sec$I20*df.sec$T0 # Y^a=0
...
1) We run the function "vary_parameters" once for each value selected for the parameter that is varied, in order to obtain the measured VE for each iteration. Results are stored in a list of data frames (hereafter, *list one*; each dataframe contains 1000 rows, each row with one iteration of the simulation with the selected value for the parameter that is being varied, therefore, the list contains 10 dataframes, each of them corresponding to 10 different values of the parameter selected). 
...
We selected a baseline prevalence for testing of ~0.28. Therefore, the adjustments for the T-intercept were designed to cover shifts in the prevalence of T of 0.05 in both directions (ie, baseline prevalence between 0.25 to 0.7). To identify the necessary values for the T-intercept, we employed a 'for' loop, iterating over several T-intercept values and calculating the corresponding prevalence of T.
...
We selected a baseline prevalence for I2 of ~0.12. Therefore, the adjustments for the T-intercept were designed to cover shifts in the prevalence of T of 0.05 in both directions (between 0.05 and 0.5). To identify the necessary values for the I2-intercept, we employed a 'for' loop, iterating over several I2-intercept values and calculating the corresponding prevalence of I2.
...
Then, in part e, we create a function to generate the plots, which uses the functions that estimate bias:
...
# Print using datatable
DT::datatable(merged_table)
...
# -----------------------------------------------------------------------------
# Final Update Date: Oct 15, 23 (12:39 hs)
# -----------------------------------------------------------------------------
